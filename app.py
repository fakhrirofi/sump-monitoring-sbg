...
if 'data_sump' not in st.session_state:
    ...
    for i in range(120):
        ...
        for site in SITE_MAP.keys():
            pit = SITE_MAP[site][0] 
            # Hujan
            rain_plan = np.random.choice([10, 20, 30, 40]) # Plan biasanya angka rata
            rain_act = np.random.choice([0, 0, 0, 5, 15, 45, 60, 10]) # Actual lebih variatif
...
# --- 5. TABS ---
tab_dash, tab_input, tab_db = st.tabs(["📊 Dashboard & Balance", "📝 Input Plan/Act", "📂 Database"])
    ...
    if df_wb_dash.empty:
        ...
        with c_left:
            ...
            fig_rain.add_trace(go.Scatter(
                ...
                name='Plan (Limit)', 
                mode='lines',
                line=dict(color='#e74c3c', width=2, dash='dash') # Garis Merah Putus-putus
            ))
...
with tab_input:
    ...
    with st.container():
        ...
        site_in = c2.selectbox("Site", list(SITE_MAP.keys()), key="s_in")
        pit_in = c3.selectbox("Pit", SITE_MAP[site_in], key="p_in")
...
with tab_db:
    ...
    dl_site = st.selectbox("Site Download:", list(SITE_MAP.keys()))
